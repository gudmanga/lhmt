...
def paskutinis_atnaujinimas(miestas):
    ...
    stringas = "Paskutinį kartą atnaujinta: " + \
        str(ltu_data(info['forecastCreationTimeUtc'])) + ","
    i = 0
    siandien = str(datetime.now())[:10]
    valandos = []
    duomenys = []
    kita = []
    while siandien == ltu_data(info['forecastTimestamps'][i]['forecastTimeUtc'])[:10]:
        valandos.append(ltu_data(
...
def lenta_duomenu(miestas):
    df = pd.read_csv(
        'https://raw.githubusercontent.com/gudmanga/lhmt/master/'+miestas + '.csv')
...
def atnaujinti_grafika(menuo, tipas, miestas, grafiko_tipas, slider_metai,):
    ...
    df = pd.read_csv(
        'https://raw.githubusercontent.com/gudmanga/lhmt/master/'+str(miestas) + '.csv')
    m = menesis[menuo]
